...
- (void)timerTicked:(NSTimer*)timer {
    ...
    NSInteger WeekdayNumber = CFAbsoluteTimeGetDayOfWeek(at, tz);
    CFGregorianDate gd = CFAbsoluteTimeGetGregorianDate(at, tz);
    NSInteger CurrentHour = gd.hour;
    
    //update the time on the display
    clockLabel.text = [NSString stringWithFormat:@"%02i:%02i", gd.hour, gd.minute];
    
    //is the time valid? (weekday, work time)
    ...
    {
        //if time not valid
        //NSLog(@"Time is NOT good: %i", WeekdayNumber);
        if (photohandler.running)
        {
            [photohandler pauseSession];
        }
    ...
    luminosityLabel.text = [NSString stringWithFormat:@"LUM: %f", photohandler.luminosity];
}
...
- (void)updateStation:(NSString *)stationAddress
{
    [self destroyStreamer];
    
    //change download source field
    downloadSourceField.text = stationAddress;
    [downloadSourceField resignFirstResponder];
...
- (IBAction)updateStationOne:(UIButton *)sender {
    NSLog(@"Station ONE Changed");
    //Soma Secret:
    [self updateStation:@"http://sfstream1.somafm.com:9012"];
}

- (IBAction)updateStationTwo:(UIButton *)sender {
    NSLog(@"Station TWO Changed");
    //Soma Groove:
    [self updateStation:@"http://sfstream1.somafm.com:9002"];
}

- (IBAction)updateStationThree:(UIButton *)sender {
    NSLog(@"Station THREE Changed");
    //Soma Indie: 
    [self updateStation:@"http://sfstream1.somafm.com:8070"];
}
